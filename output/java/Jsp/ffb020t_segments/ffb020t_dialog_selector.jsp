...
function <%=selectFunction%>(){
  $('#dlg-ffb020t').dialog('open');
  dlgFfb020tReset();
}
<%}%>
function dlgFfb020tSelected(){
  var rows = $('#dlg-dg-ffb020t').datagrid('getSelections');
  if (rows.length == 0){
  ...
  $('#dlg-ffb020t').dialog('close');
}
$(function(){
  if ($('#dlg-ffb020t').length == 0) return;
<%
  String[] targets = bindTargets.split(",");
  for(int i=0; i<targets.length; i++){
%>  bindTextboxClick('#<%= targets[i].trim()%>', '#dlg-ffb020t');
<%}%>
});
</script>
<div id="dlg-ffb020t" class="easyui-dialog pop-dialog" buttons="#dlg-ffb020t-selector-buttons" title="选择" closed="true"
data-options="
onOpen: function(){
  <%if(!"".equals(onOpen)) {%><%=onOpen%>(textboxClickSender);
  <%}else{%>dlgFfb020tReset();<%}%>
}"
>
  <div class="easyui-panel query-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
    ...
    function dlgFfb020tQuery(){
      ...
      $('#dlg-dg-ffb020t').datagrid('reload', queryParams);
    }
    function createFfb020tQueryParams(){
      var params = createQueryParams("dlg_ffb020t_query");
      params['query.f0.isNotNullValue'] = 'true';
      return params;
    }
    function dlgFfb020tReset(){
      resetQueryInputs("dlg_ffb020t_query");
      dlgFfb020tQuery();
    }
    </script>
    <div id="dlg_ffb020t_query" class="query">
...
<%= hiddenTableFields!=null? ui.createDataGridColumn(hiddenTableFields, false) : "" %>
    ...
    <table id="dlg-dg-ffb020t"></table>
  </div>
  <div id="dlg-ffb020t-selector-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="dlgFfb020tSelected()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-ffb020t').dialog('close')">取消</a>
  </div>
</div>
