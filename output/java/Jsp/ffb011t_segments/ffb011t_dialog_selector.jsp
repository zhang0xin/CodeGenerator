...
function <%=selectFunction%>(){
  $('#dlg-ffb011t').dialog('open');
  dlgFfb011tReset();
}
<%}%>
function dlgFfb011tSelected(){
  var rows = $('#dlg-dg-ffb011t').datagrid('getSelections');
  if (rows.length == 0){
  ...
  $('#dlg-ffb011t').dialog('close');
}
$(function(){
  if ($('#dlg-ffb011t').length == 0) return;
<%
  String[] targets = bindTargets.split(",");
  for(int i=0; i<targets.length; i++){
%>  bindTextboxClick('#<%= targets[i].trim()%>', '#dlg-ffb011t');
<%}%>
});
</script>
<div id="dlg-ffb011t" class="easyui-dialog pop-dialog" buttons="#dlg-ffb011t-selector-buttons" title="选择" closed="true"
data-options="
onOpen: function(){
  <%if(!"".equals(onOpen)) {%><%=onOpen%>(textboxClickSender);
  <%}else{%>dlgFfb011tReset();<%}%>
}"
>
  <div class="easyui-panel query-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
    ...
    function dlgFfb011tQuery(){
      ...
      $('#dlg-dg-ffb011t').datagrid('reload', queryParams);
    }
    function createFfb011tQueryParams(){
      var params = createQueryParams("dlg_ffb011t_query");
      params['query.f0.isNotNullValue'] = 'true';
      return params;
    }
    function dlgFfb011tReset(){
      resetQueryInputs("dlg_ffb011t_query");
      dlgFfb011tQuery();
    }
    </script>
    <div id="dlg_ffb011t_query" class="query">
...
<%= hiddenTableFields!=null? ui.createDataGridColumn(hiddenTableFields, false) : "" %>
    ...
    <table id="dlg-dg-ffb011t"></table>
  </div>
  <div id="dlg-ffb011t-selector-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="dlgFfb011tSelected()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-ffb011t').dialog('close')">取消</a>
  </div>
</div>
