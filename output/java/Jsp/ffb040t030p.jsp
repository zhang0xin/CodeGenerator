...
<html>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      ...
      $('#icon-add').bind("click",function(){newFfb040t030pEntity();});
      $('#icon-edit').bind("click",function(){editFfb040t030pEntity();});
      $('#icon-remove').bind("click",function(){deleteFfb040t030pEntity();});
      $('#icon-search').bind("click",function(){queryFfb040tEntity();});
      $('#icon-cancel').bind("click",function(){resetFfb040tQuery();});
      $('#icon-export').bind("click",function(){exportFfb040t030pEntities();});
      $('#icon-file-lock').bind("click",function(){archiveFfb040t030pEntities();});
      $('#icon-file-unlock').bind("click",function(){activeFfb040t030pEntities();});
      
    });
    
    function newFfb040t030pEntity(){
      $('#dlgFfb040t030pEntity').dialog('open').dialog('setTitle', '新建');
      $('#frmFfb040tEntity').form('clear');
    }
    
    function editFfb040t030pEntity(){
      var r = $('#dgFfb040t030p').datagrid('getSelected');
      if(r==null){
      ...
      $('#dlgFfb040t030pEntity').dialog('open').dialog('setTitle', '编辑');
    }
    
    function saveFfb040t030pEntity(){
      if (!$("#frmFfb040tEntity").form("validate")){
        $.messager.alert("系统提示", "请确保录入字段都符合正确的格式或录入了所有的必填字段，查找表单中红色部分并修改。")
        return;
      }
      var row = $("#dgFfb040t030p").datagrid("getSelected")
      if (($("dlgFfb040t030pEntity input[name='entity.f0']").val() == "")) row = {};
      var entity = createEntityData("frmFfb040tEntity", row)
      $.post("ffb040t030p.action?reqtype=save", entity, function(data){
        if (data.success){
          $("#frmFfb040tEntity").form("clear");
          $("#dlgFfb040t030pEntity").dialog("close");
          $("#dgFfb040t030p").datagrid("reload");
        }else{
    ...
    function deleteFfb040t030pEntity(){
      executeCommand("ffb040t", "delete", "dgFfb040t030p", "请选择要删除的行!", true, "确认删除吗？")
    }
    
    function archiveFfb040t030pEntities(){ 
      executeCommand("ffb040t030p", "archive", "dgFfb040t030p", "请选择要归档的行!")
    }
    
    function activeFfb040t030pEntities(){
      executeCommand("ffb040t030p", "active", "dgFfb040t030p", "请选择要激活的行!") 
    }
    
    function exportFfb040t030pEntities(){
      var queryParams = createFfb040tQueryParams();
      queryParams.selectedColumns = getColumnDescriptions($('#dgFfb040t030p'));
    ...
    function onFfb040t030pDblClickRow(rowIndex, rowData){
      if ($('#icon-edit').length == 0) return;
      editFfb040t030pEntity();
    }
    
    function onFfb040t030pClickRow(rowIndex, rowData){
      var r = $('#dgFfb040t030p').datagrid('getSelected');
      loadFfb040tEntity(r);
  ...
  <body style="margin:0;padding:0px;">
    ...
    <div class="easyui-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
      <jsp:include page="ffb040t_segments/ffb040t_query.jsp" >
        <jsp:param name="displayQueryFields" value="purchaseBatchId, unloadPortId, shipAgent" />
        <jsp:param name="targetDataGridId" value="dgFfb040t030p" />
      </jsp:include>
    </div>
    <div class="easyui-panel" title="卸港滞期速遣" data-options="iconCls:'icon-edit',collapsible:true">
      <jsp:include page="ffb040t_segments/ffb040t_grid.jsp" >
        <jsp:param name="displayTableFields" value="purchaseBatchId, unloadPortId, unloadDelayDays, unloadDelayFee, unloadDelayFeeUnit, unloadDelayFeeRate, unloadTransportAdjust, unloadTransportAdjustUnit, speedDelayFee, speedDelayFeeUnit, shipAgent" />
        <jsp:param name="onDblClickRow" value="function(rowIndex, rowData){ onFfb040t030pDblClickRow(rowIndex, rowData); }" />
        <jsp:param name="onClickRow" value="function(rowIndex, rowData){ onFfb040t030pClickRow(rowIndex, rowData); }" />
        <jsp:param name="dataGridId" value="dgFfb040t030p" />
        <jsp:param name="hiddenCheckboxColumn" value="false" />
      </jsp:include> 
    </div>

    <div id="dlgFfb040t030pEntity" class="easyui-dialog edit-dialog" buttons="#dlg-buttons" title="新建" closed="true" >
      <jsp:include page="ffb040t_segments/ffb040t_edit.jsp" >
        <jsp:param name="displayEditFields" value="unloadDelayDays, unloadDelayFeeWithUnit, unloadDelayFeeRate, unloadTransportAdjustWithUnit, speedDelayFeeWithUnit, shipAgent" />
        <jsp:param name="formId" value="frmEntity" />
      </jsp:include> 
      <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveFfb040t030pEntity()">保存</a> 
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgFfb040t030pEntity').dialog('close')">取消</a>
      </div>
    </div>
