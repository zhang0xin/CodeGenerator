...
<html>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      ...
      $('#icon-add').bind("click",function(){newFfb030t020pEntity();});
      $('#icon-edit').bind("click",function(){editFfb030t020pEntity();});
      $('#icon-remove').bind("click",function(){deleteFfb030t020pEntity();});
      $('#icon-search').bind("click",function(){queryFfb030tEntity();});
      $('#icon-cancel').bind("click",function(){resetFfb030tQuery();});
      $('#icon-export').bind("click",function(){exportFfb030t020pEntities();});
      $('#icon-file-lock').bind("click",function(){archiveFfb030t020pEntities();});
      $('#icon-file-unlock').bind("click",function(){activeFfb030t020pEntities();});
      
    });
    
    function newFfb030t020pEntity(){
      $('#dlgFfb030t020pEntity').dialog('open').dialog('setTitle', '新建');
      $('#frmFfb030tEntity').form('clear');
    }
    
    function editFfb030t020pEntity(){
      var r = $('#dgFfb030t020p').datagrid('getSelected');
      if(r==null){
      ...
      $('#dlgFfb030t020pEntity').dialog('open').dialog('setTitle', '编辑');
    }
    
    function saveFfb030t020pEntity(){
      if (!$("#frmFfb030tEntity").form("validate")){
        $.messager.alert("系统提示", "请确保录入字段都符合正确的格式或录入了所有的必填字段，查找表单中红色部分并修改。")
        return;
      }
      var row = $("#dgFfb030t020p").datagrid("getSelected")
      if (($("dlgFfb030t020pEntity input[name='entity.f0']").val() == "")) row = {};
      var entity = createEntityData("frmFfb030tEntity", row)
      $.post("ffb030t020p.action?reqtype=save", entity, function(data){
        if (data.success){
          $("#frmFfb030tEntity").form("clear");
          $("#dlgFfb030t020pEntity").dialog("close");
          $("#dgFfb030t020p").datagrid("reload");
        }else{
    ...
    function deleteFfb030t020pEntity(){
      executeCommand("ffb030t", "delete", "dgFfb030t020p", "请选择要删除的行!", true, "确认删除吗？")
    }
    
    function archiveFfb030t020pEntities(){ 
      executeCommand("ffb030t020p", "archive", "dgFfb030t020p", "请选择要归档的行!")
    }
    
    function activeFfb030t020pEntities(){
      executeCommand("ffb030t020p", "active", "dgFfb030t020p", "请选择要激活的行!") 
    }
    
    function exportFfb030t020pEntities(){
      var queryParams = createFfb030tQueryParams();
      queryParams.selectedColumns = getColumnDescriptions($('#dgFfb030t020p'));
    ...
    function onFfb030t020pDblClickRow(rowIndex, rowData){
      if ($('#icon-edit').length == 0) return;
      editFfb030t020pEntity();
    }
    
    function onFfb030t020pClickRow(rowIndex, rowData){
      var r = $('#dgFfb030t020p').datagrid('getSelected');
      loadFfb030tEntity(r);
  ...
  <body style="margin:0;padding:0px;">
    ...
    <div class="easyui-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
      <jsp:include page="ffb030t_segments/ffb030t_query.jsp" >
        ...
        <jsp:param name="targetDataGridId" value="dgFfb030t020p" />
      </jsp:include>
    </div>
    <div class="easyui-panel" title="卸港滞期速遣" data-options="iconCls:'icon-edit',collapsible:true">
      <jsp:include page="ffb030t_segments/ffb030t_grid.jsp" >
        ...
        <jsp:param name="onDblClickRow" value="function(rowIndex, rowData){ onFfb030t020pDblClickRow(rowIndex, rowData); }" />
        <jsp:param name="onClickRow" value="function(rowIndex, rowData){ onFfb030t020pClickRow(rowIndex, rowData); }" />
        <jsp:param name="dataGridId" value="dgFfb030t020p" />
        <jsp:param name="hiddenCheckboxColumn" value="false" />
      </jsp:include> 
    </div>

    <div id="dlgFfb030t020pEntity" class="easyui-dialog edit-dialog" buttons="#dlg-buttons" title="新建" closed="true" >
      ...
      <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveFfb030t020pEntity()">保存</a> 
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgFfb030t020pEntity').dialog('close')">取消</a>
      </div>
    </div>
