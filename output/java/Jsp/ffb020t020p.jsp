...
<html>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      ...
      $('#icon-add').bind("click",function(){newFfb020t020pEntity();});
      $('#icon-edit').bind("click",function(){editFfb020t020pEntity();});
      $('#icon-remove').bind("click",function(){deleteFfb020t020pEntity();});
      $('#icon-search').bind("click",function(){queryFfb020tEntity();});
      $('#icon-cancel').bind("click",function(){resetFfb020tQuery();});
      $('#icon-export').bind("click",function(){exportFfb020t020pEntities();});
      $('#icon-file-lock').bind("click",function(){archiveFfb020t020pEntities();});
      $('#icon-file-unlock').bind("click",function(){activeFfb020t020pEntities();});
    ...
    function newFfb020t020pEntity(){
      $('#dlgFfb020t020pEntity').dialog('open').dialog('setTitle', '新建');
      $('#frmFfb020tEntity').form('clear');
    }
    
    function editFfb020t020pEntity(){
      var r = $('#dgFfb020t020p').datagrid('getSelected');
      if(r==null){
      ...
      $('#dlgFfb020t020pEntity').dialog('open').dialog('setTitle', '编辑');
    }
    
    function saveFfb020t020pEntity(){
      if (!$("#frmFfb020tEntity").form("validate")){
        $.messager.alert("系统提示", "请确保录入字段都符合正确的格式或录入了所有的必填字段，查找表单中红色部分并修改。")
        return;
      }
      var row = $("#dgFfb020t020p").datagrid("getSelected")
      if (($("dlgFfb020t020pEntity input[name='entity.f0']").val() == "")) row = {};
      var entity = createEntityData("frmFfb020tEntity", row)
      $.post("ffb020t020p.action?reqtype=save", entity, function(data){
        if (data.success){
          $("#frmFfb020tEntity").form("clear");
          $("#dlgFfb020t020pEntity").dialog("close");
          $("#dgFfb020t020p").datagrid("reload");
        }else{
    ...
    function deleteFfb020t020pEntity(){
      executeCommand("ffb020t", "delete", "dgFfb020t020p", "请选择要删除的行!", true, "确认删除吗？")
    }
    
    function archiveFfb020t020pEntities(){ 
      executeCommand("ffb020t020p", "archive", "dgFfb020t020p", "请选择要归档的行!")
    }
    
    function activeFfb020t020pEntities(){
      executeCommand("ffb020t020p", "active", "dgFfb020t020p", "请选择要激活的行!") 
    }
    
    function exportFfb020t020pEntities(){
      var queryParams = createFfb020tQueryParams();
      queryParams.selectedColumns = getColumnDescriptions($('#dgFfb020t020p'));
    ...
    function onFfb020t020pDblClickRow(rowIndex, rowData){
      if ($('#icon-edit').length == 0) return;
      editFfb020t020pEntity();
    }
    
    function onFfb020t020pClickRow(rowIndex, rowData){
      var r = $('#dgFfb020t020p').datagrid('getSelected');
      loadFfb020tEntity(r);
  ...
  <body style="margin:0;padding:0px;">
    ...
    <div class="easyui-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
      <jsp:include page="ffb020t_segments/ffb020t_query.jsp" >
        <jsp:param name="displayQueryFields" value="shipBatchId, shipNumber" />
        <jsp:param name="targetDataGridId" value="dgFfb020t020p" />
      </jsp:include>
    </div>
    <div class="easyui-panel" title="信用证" data-options="iconCls:'icon-edit',collapsible:true">
      <jsp:include page="ffb020t_segments/ffb020t_grid.jsp" >
        <jsp:param name="displayTableFields" value="shipBatchId, shipNumber" />
        <jsp:param name="onDblClickRow" value="function(rowIndex, rowData){ onFfb020t020pDblClickRow(rowIndex, rowData); }" />
        <jsp:param name="onClickRow" value="function(rowIndex, rowData){ onFfb020t020pClickRow(rowIndex, rowData); }" />
        <jsp:param name="dataGridId" value="dgFfb020t020p" />
        <jsp:param name="hiddenCheckboxColumn" value="false" />
      </jsp:include> 
    </div>

    <div id="dlgFfb020t020pEntity" class="easyui-dialog edit-dialog" buttons="#dlg-buttons" title="新建" closed="true" >
      ...
      <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveFfb020t020pEntity()">保存</a> 
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgFfb020t020pEntity').dialog('close')">取消</a>
      </div>
    </div>
    ...
    <div class="easyui-panel" title="采购资源" data-options="iconCls:'icon-note',collapsible:true">
      <jsp:include page="ffb010t_segments/ffb010t_details_editor.jsp" >
        ...
        <jsp:param name="targetMainGridId" value="dgFfb020t020p" />
