...
function <%=selectFunction%>(){
  $('#dlg-ffb011v').dialog('open');
  dlgFfb011vReset();
}
<%}%>
function dlgFfb011vSelected(){
  var rows = $('#dlg-dg-ffb011v').datagrid('getSelections');
  if (rows.length == 0){
  ...
  $('#dlg-ffb011v').dialog('close');
}
$(function(){
  if ($('#dlg-ffb011v').length == 0) return;
<%
  String[] targets = bindTargets.split(",");
  for(int i=0; i<targets.length; i++){
%>  bindTextboxClick('#<%= targets[i].trim()%>', '#dlg-ffb011v');
<%}%>
});
</script>
<div id="dlg-ffb011v" class="easyui-dialog pop-dialog" buttons="#dlg-ffb011v-selector-buttons" title="选择" closed="true"
data-options="
onOpen: function(){
  <%if(!"".equals(onOpen)) {%><%=onOpen%>(textboxClickSender);
  <%}else{%>dlgFfb011vReset();<%}%>
}"
>
  <div class="easyui-panel query-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
    ...
    function dlgFfb011vQuery(){
      ...
      $('#dlg-dg-ffb011v').datagrid('reload', queryParams);
    }
    function createFfb011vQueryParams(){
      var params = createQueryParams("dlg_ffb011v_query");
      params['query.f0.isNotNullValue'] = 'true';
      return params;
    }
    function dlgFfb011vReset(){
      resetQueryInputs("dlg_ffb011v_query");
      dlgFfb011vQuery();
    }
    </script>
    <div id="dlg_ffb011v_query" class="query">
...
<%= hiddenTableFields!=null? ui.createDataGridColumn(hiddenTableFields, false) : "" %>
    ...
    <table id="dlg-dg-ffb011v"></table>
  </div>
  <div id="dlg-ffb011v-selector-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="dlgFfb011vSelected()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-ffb011v').dialog('close')">取消</a>
  </div>
</div>
