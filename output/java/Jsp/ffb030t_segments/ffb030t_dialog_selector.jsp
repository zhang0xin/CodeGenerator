...
function <%=selectFunction%>(){
  $('#dlg-ffb030t').dialog('open');
  dlgFfb030tReset();
}
<%}%>
function dlgFfb030tSelected(){
  var rows = $('#dlg-dg-ffb030t').datagrid('getSelections');
  if (rows.length == 0){
  ...
  $('#dlg-ffb030t').dialog('close');
}
$(function(){
  if ($('#dlg-ffb030t').length == 0) return;
<%
  String[] targets = bindTargets.split(",");
  for(int i=0; i<targets.length; i++){
%>  bindTextboxClick('#<%= targets[i].trim()%>', '#dlg-ffb030t');
<%}%>
});
</script>
<div id="dlg-ffb030t" class="easyui-dialog pop-dialog" buttons="#dlg-ffb030t-selector-buttons" title="选择" closed="true"
data-options="
onOpen: function(){
  <%if(!"".equals(onOpen)) {%><%=onOpen%>(textboxClickSender);
  <%}else{%>dlgFfb030tReset();<%}%>
}"
>
  <div class="easyui-panel query-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
    ...
    function dlgFfb030tQuery(){
      ...
      $('#dlg-dg-ffb030t').datagrid('reload', queryParams);
    }
    function createFfb030tQueryParams(){
      var params = createQueryParams("dlg_ffb030t_query");
      params['query.f0.isNotNullValue'] = 'true';
      return params;
    }
    function dlgFfb030tReset(){
      resetQueryInputs("dlg_ffb030t_query");
      dlgFfb030tQuery();
    }
    </script>
    <div id="dlg_ffb030t_query" class="query">
...
<%= hiddenTableFields!=null? ui.createDataGridColumn(hiddenTableFields, false) : "" %>
    ...
    <table id="dlg-dg-ffb030t"></table>
  </div>
  <div id="dlg-ffb030t-selector-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="dlgFfb030tSelected()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-ffb030t').dialog('close')">取消</a>
  </div>
</div>
