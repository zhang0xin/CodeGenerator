...
function <%=selectFunction%>(){
  $('#dlg-student').dialog('open');
  dlgStudentReset();
}
<%}%>
function dlgStudentSelected(){
  var rows = $('#dlg-dg-student').datagrid('getSelections');
  if (rows.length == 0){
...
$(function(){
  if ($('#dlg-student').length == 0) return;
<%
  String[] targets = bindTargets.split(",");
  for(int i=0; i<targets.length; i++){
%>  bindTextboxClick('#<%= targets[i].trim()%>', '#dlg-student');
<%}%>
});
</script>
<div id="dlg-student" class="easyui-dialog pop-dialog" buttons="#dlg-student-selector-buttons" title="选择" closed="true"
data-options="
onOpen: function(){
  <%if(!"".equals(onOpen)) {%><%=onOpen%>(textboxClickSender);
  <%}else{%>dlgStudentReset();<%}%>
}"
>
  <div class="easyui-panel query-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
    ...
    function dlgStudentQuery(){
      ...
      $('#dlg-dg-student').datagrid('reload', queryParams);
    }
    function createStudentQueryParams(){
      var params = createQueryParams("dlg_student_query");
      params['query.f0.isNotNullValue'] = 'true';
      return params;
    }
    function dlgStudentReset(){
      resetQueryInputs("dlg_student_query");
      dlgStudentQuery();
    }
    </script>
    <div id="dlg_student_query" class="query">
...
<%= hiddenTableFields!=null? ui.createDataGridColumn(hiddenTableFields, false) : "" %>
    ...
    <table id="dlg-dg-student"></table>
  </div>
  <div id="dlg-student-selector-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="dlgStudentSelected()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-student').dialog('close')">取消</a>
  </div>
</div>
