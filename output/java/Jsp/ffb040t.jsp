...
<html>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      ...
      $('#icon-add').bind("click",function(){newFfb040tEntity();});
      $('#icon-edit').bind("click",function(){editFfb040tEntity();});
      $('#icon-remove').bind("click",function(){deleteFfb040tEntity();});
      $('#icon-search').bind("click",function(){queryFfb040tEntity();});
      $('#icon-cancel').bind("click",function(){resetFfb040tQuery();});
      $('#icon-export').bind("click",function(){exportFfb040tEntities();});
      $('#icon-file-lock').bind("click",function(){archiveFfb040tEntities();});
      $('#icon-file-unlock').bind("click",function(){activeFfb040tEntities();});
      
    });
    
    function newFfb040tEntity(){
      $('#dlgFfb040tEntity').dialog('open').dialog('setTitle', '新建');
      $('#frmFfb040tEntity').form('clear');
    }
    
    function editFfb040tEntity(){
      var r = $('#dgFfb040t').datagrid('getSelected');
      if(r==null){
      ...
      $('#dlgFfb040tEntity').dialog('open').dialog('setTitle', '编辑');
    }
    
    function saveFfb040tEntity(){
      if (!$("#frmFfb040tEntity").form("validate")){
        $.messager.alert("系统提示", "请确保录入字段都符合正确的格式或录入了所有的必填字段，查找表单中红色部分并修改。")
        return;
      }
      var row = $("#dgFfb040t").datagrid("getSelected")
      if (($("dlgFfb040tEntity input[name='entity.f0']").val() == "")) row = {};
      var entity = createEntityData("frmFfb040tEntity", row)
      $.post("ffb040t.action?reqtype=save", entity, function(data){
        if (data.success){
          $("#frmFfb040tEntity").form("clear");
          $("#dlgFfb040tEntity").dialog("close");
          $("#dgFfb040t").datagrid("reload");
        }else{
    ...
    function deleteFfb040tEntity(){
      executeCommand("ffb040t", "delete", "dgFfb040t", "请选择要删除的行!", true, "确认删除吗？")
    }
    
    function archiveFfb040tEntities(){ 
      executeCommand("ffb040t", "archive", "dgFfb040t", "请选择要归档的行!")
    }
    
    function activeFfb040tEntities(){
      executeCommand("ffb040t", "active", "dgFfb040t", "请选择要激活的行!") 
    }
    
    function exportFfb040tEntities(){
      var queryParams = createFfb040tQueryParams();
      queryParams.selectedColumns = getColumnDescriptions($('#dgFfb040t'));
    ...
    function onFfb040tDblClickRow(rowIndex, rowData){
      if ($('#icon-edit').length == 0) return;
      editFfb040tEntity();
    }
    
    function onFfb040tClickRow(rowIndex, rowData){
      var r = $('#dgFfb040t').datagrid('getSelected');
      loadFfb040tEntity(r);
  ...
  <body style="margin:0;padding:0px;">
    ...
    <div class="easyui-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
      <jsp:include page="ffb040t_segments/ffb040t_query.jsp" >
        <jsp:param name="displayQueryFields" value="purchaseBatchId, unloadPortId, tfe, sio2, cao, mgo, al2o3, sio2Al2o3, tio2, k2o, na2o, zno, as, pbo, cr, ni, cu, mn, h2o, feo, p, s, burnLose, wetWeight, grainSize, remark, createBy, createDate, updateBy, updateDate, archived, portFee, portConstructionFee, detectFee, inspectionFee, portAgentFee, sampleFee, waterGaugeFee, boatFee, declareAgentFee, declareInspAgentFee, declareCustomsAgent, freightAgent, finalReceiver, ffb040t050pArchived, unloadDelayDays, unloadDelayFee, unloadDelayFeeUnit, unloadDelayFeeRate, unloadTransportAdjust, unloadTransportAdjustUnit, speedDelayFee, speedDelayFeeUnit, shipAgent, ffb040t030pArchived, goodsAgent, declareCustomsCompany, depositFee, depositFeeUnit, declareCustoms, customsClearanceLastDate, customsClearanceActualDate, safeDeclar, declareCustomsDate, declareCustomsBill, customsClearanceState, declareCustomsFee, declareCustomsFeeUnit, licenseNumber, valueAddedTax, valueAddedTaxUnit, customsTaxEqualize, customsTaxEqualizeUnit, ffb040t040pArchived" />
        <jsp:param name="targetDataGridId" value="dgFfb040t" />
      </jsp:include>
    </div>
    <div class="easyui-panel" title="品质" data-options="iconCls:'icon-edit',collapsible:true">
      <jsp:include page="ffb040t_segments/ffb040t_grid.jsp" >
        <jsp:param name="displayTableFields" value="purchaseBatchId, unloadPortId, tfe, sio2, cao, mgo, al2o3, sio2Al2o3, tio2, k2o, na2o, zno, as, pbo, cr, ni, cu, mn, h2o, feo, p, s, burnLose, wetWeight, grainSize, remark, createBy, createDate, updateBy, updateDate, archived, portFee, portConstructionFee, detectFee, inspectionFee, portAgentFee, sampleFee, waterGaugeFee, boatFee, declareAgentFee, declareInspAgentFee, declareCustomsAgent, freightAgent, finalReceiver, ffb040t050pArchived, unloadDelayDays, unloadDelayFee, unloadDelayFeeUnit, unloadDelayFeeRate, unloadTransportAdjust, unloadTransportAdjustUnit, speedDelayFee, speedDelayFeeUnit, shipAgent, ffb040t030pArchived, goodsAgent, declareCustomsCompany, depositFee, depositFeeUnit, declareCustoms, customsClearanceLastDate, customsClearanceActualDate, safeDeclar, declareCustomsDate, declareCustomsBill, customsClearanceState, declareCustomsFee, declareCustomsFeeUnit, licenseNumber, valueAddedTax, valueAddedTaxUnit, customsTaxEqualize, customsTaxEqualizeUnit, ffb040t040pArchived" />
        <jsp:param name="onDblClickRow" value="function(rowIndex, rowData){ onFfb040tDblClickRow(rowIndex, rowData); }" />
        <jsp:param name="onClickRow" value="function(rowIndex, rowData){ onFfb040tClickRow(rowIndex, rowData); }" />
        <jsp:param name="dataGridId" value="dgFfb040t" />
        <jsp:param name="hiddenCheckboxColumn" value="false" />
      </jsp:include> 
    </div>

    <div id="dlgFfb040tEntity" class="easyui-dialog edit-dialog" buttons="#dlg-buttons" title="新建" closed="true" >
      <jsp:include page="ffb040t_segments/ffb040t_edit.jsp" >
        <jsp:param name="displayEditFields" value="purchaseBatchId, unloadPortId, tfe, sio2, cao, mgo, al2o3, sio2Al2o3, tio2, k2o, na2o, zno, as, pbo, cr, ni, cu, mn, h2o, feo, p, s, burnLose, wetWeight, grainSize, remark, createBy, createDate, updateBy, updateDate, archived, portFee, portConstructionFee, detectFee, inspectionFee, portAgentFee, sampleFee, waterGaugeFee, boatFee, declareAgentFee, declareInspAgentFee, declareCustomsAgent, freightAgent, finalReceiver, ffb040t050pArchived, unloadDelayDays, unloadDelayFeeWithUnit, unloadDelayFeeUnit, unloadDelayFeeRate, unloadTransportAdjustWithUnit, unloadTransportAdjustUnit, speedDelayFeeWithUnit, speedDelayFeeUnit, shipAgent, ffb040t030pArchived, goodsAgent, declareCustomsCompany, depositFeeWithUnit, depositFeeUnit, declareCustoms, customsClearanceLastDate, customsClearanceActualDate, safeDeclar, declareCustomsDate, declareCustomsBill, customsClearanceState, declareCustomsFeeWithUnit, declareCustomsFeeUnit, licenseNumber, valueAddedTaxWithUnit, valueAddedTaxUnit, customsTaxEqualizeWithUnit, customsTaxEqualizeUnit, ffb040t040pArchived" />
        <jsp:param name="formId" value="frmEntity" />
      </jsp:include> 
      <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveFfb040tEntity()">保存</a> 
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgFfb040tEntity').dialog('close')">取消</a>
      </div>
    </div>
