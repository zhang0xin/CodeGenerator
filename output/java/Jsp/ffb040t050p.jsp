...
<html>
  ...
  <script type="text/javascript">
    ...
    $(function(){
      ...
      $('#icon-add').bind("click",function(){newFfb040t050pEntity();});
      $('#icon-edit').bind("click",function(){editFfb040t050pEntity();});
      $('#icon-remove').bind("click",function(){deleteFfb040t050pEntity();});
      $('#icon-search').bind("click",function(){queryFfb040tEntity();});
      $('#icon-cancel').bind("click",function(){resetFfb040tQuery();});
      $('#icon-export').bind("click",function(){exportFfb040t050pEntities();});
      $('#icon-file-lock').bind("click",function(){archiveFfb040t050pEntities();});
      $('#icon-file-unlock').bind("click",function(){activeFfb040t050pEntities();});
      
    });
    
    function newFfb040t050pEntity(){
      $('#dlgFfb040t050pEntity').dialog('open').dialog('setTitle', '新建');
      $('#frmFfb040tEntity').form('clear');
    }
    
    function editFfb040t050pEntity(){
      var r = $('#dgFfb040t050p').datagrid('getSelected');
      if(r==null){
      ...
      $('#dlgFfb040t050pEntity').dialog('open').dialog('setTitle', '编辑');
    }
    
    function saveFfb040t050pEntity(){
      if (!$("#frmFfb040tEntity").form("validate")){
        $.messager.alert("系统提示", "请确保录入字段都符合正确的格式或录入了所有的必填字段，查找表单中红色部分并修改。")
        return;
      }
      var row = $("#dgFfb040t050p").datagrid("getSelected")
      if (($("dlgFfb040t050pEntity input[name='entity.f0']").val() == "")) row = {};
      var entity = createEntityData("frmFfb040tEntity", row)
      $.post("ffb040t050p.action?reqtype=save", entity, function(data){
        if (data.success){
          $("#frmFfb040tEntity").form("clear");
          $("#dlgFfb040t050pEntity").dialog("close");
          $("#dgFfb040t050p").datagrid("reload");
        }else{
    ...
    function deleteFfb040t050pEntity(){
      executeCommand("ffb040t", "delete", "dgFfb040t050p", "请选择要删除的行!", true, "确认删除吗？")
    }
    
    function archiveFfb040t050pEntities(){ 
      executeCommand("ffb040t050p", "archive", "dgFfb040t050p", "请选择要归档的行!")
    }
    
    function activeFfb040t050pEntities(){
      executeCommand("ffb040t050p", "active", "dgFfb040t050p", "请选择要激活的行!") 
    }
    
    function exportFfb040t050pEntities(){
      var queryParams = createFfb040tQueryParams();
      queryParams.selectedColumns = getColumnDescriptions($('#dgFfb040t050p'));
    ...
    function onFfb040t050pDblClickRow(rowIndex, rowData){
      if ($('#icon-edit').length == 0) return;
      editFfb040t050pEntity();
    }
    
    function onFfb040t050pClickRow(rowIndex, rowData){
      var r = $('#dgFfb040t050p').datagrid('getSelected');
      loadFfb040tEntity(r);
  ...
  <body style="margin:0;padding:0px;">
    ...
    <div class="easyui-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
      <jsp:include page="ffb040t_segments/ffb040t_query.jsp" >
        <jsp:param name="displayQueryFields" value="declareCustomsAgent, freightAgent, finalReceiver" />
        <jsp:param name="targetDataGridId" value="dgFfb040t050p" />
      </jsp:include>
    </div>
    <div class="easyui-panel" title="卸港口费" data-options="iconCls:'icon-edit',collapsible:true">
      <jsp:include page="ffb040t_segments/ffb040t_grid.jsp" >
        <jsp:param name="displayTableFields" value="portFee, portConstructionFee, detectFee, inspectionFee, portAgentFee, sampleFee, waterGaugeFee, boatFee, declareAgentFee, declareInspAgentFee, declareCustomsAgent, freightAgent, finalReceiver" />
        <jsp:param name="onDblClickRow" value="function(rowIndex, rowData){ onFfb040t050pDblClickRow(rowIndex, rowData); }" />
        <jsp:param name="onClickRow" value="function(rowIndex, rowData){ onFfb040t050pClickRow(rowIndex, rowData); }" />
        <jsp:param name="dataGridId" value="dgFfb040t050p" />
        <jsp:param name="hiddenCheckboxColumn" value="false" />
      </jsp:include> 
    </div>

    <div id="dlgFfb040t050pEntity" class="easyui-dialog edit-dialog" buttons="#dlg-buttons" title="新建" closed="true" >
      <jsp:include page="ffb040t_segments/ffb040t_edit.jsp" >
        <jsp:param name="displayEditFields" value="portFee, portConstructionFee, detectFee, inspectionFee, portAgentFee, sampleFee, waterGaugeFee, boatFee, declareAgentFee, declareInspAgentFee, declareCustomsAgent, freightAgent, finalReceiver" />
        <jsp:param name="formId" value="frmEntity" />
      </jsp:include> 
      <div id="dlg-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveFfb040t050pEntity()">保存</a> 
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript:$('#dlgFfb040t050pEntity').dialog('close')">取消</a>
      </div>
    </div>
