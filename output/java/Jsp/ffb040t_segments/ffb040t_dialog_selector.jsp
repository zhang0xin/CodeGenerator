...
function <%=selectFunction%>(){
  $('#dlg-ffb040t').dialog('open');
  dlgFfb040tReset();
}
<%}%>
function dlgFfb040tSelected(){
  var rows = $('#dlg-dg-ffb040t').datagrid('getSelections');
  if (rows.length == 0){
  ...
  $('#dlg-ffb040t').dialog('close');
}
$(function(){
  if ($('#dlg-ffb040t').length == 0) return;
<%
  String[] targets = bindTargets.split(",");
  for(int i=0; i<targets.length; i++){
%>  bindTextboxClick('#<%= targets[i].trim()%>', '#dlg-ffb040t');
<%}%>
});
</script>
<div id="dlg-ffb040t" class="easyui-dialog pop-dialog" buttons="#dlg-ffb040t-selector-buttons" title="选择" closed="true"
data-options="
onOpen: function(){
  <%if(!"".equals(onOpen)) {%><%=onOpen%>(textboxClickSender);
  <%}else{%>dlgFfb040tReset();<%}%>
}"
>
  <div class="easyui-panel query-panel" title="查询选项" data-options="iconCls:'icon-search',collapsible:true,collapsed:true">
    ...
    function dlgFfb040tQuery(){
      ...
      $('#dlg-dg-ffb040t').datagrid('reload', queryParams);
    }
    function createFfb040tQueryParams(){
      var params = createQueryParams("dlg_ffb040t_query");
      params['query.f0.isNotNullValue'] = 'true';
      return params;
    }
    function dlgFfb040tReset(){
      resetQueryInputs("dlg_ffb040t_query");
      dlgFfb040tQuery();
    }
    </script>
    <div id="dlg_ffb040t_query" class="query">
...
<%= hiddenTableFields!=null? ui.createDataGridColumn(hiddenTableFields, false) : "" %>
    ...
    <table id="dlg-dg-ffb040t"></table>
  </div>
  <div id="dlg-ffb040t-selector-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="dlgFfb040tSelected()">确定</a> 
    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-cancel" onclick="$('#dlg-ffb040t').dialog('close')">取消</a>
  </div>
</div>
